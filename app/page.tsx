  ...
  return (
    <div className="grid md:grid-cols-12 gap-4">
      <aside className="md:col-span-3">
        <div className="bg-zinc-900 border border-zinc-800 rounded-2xl sticky top-4 p-0">
      ...
      <div className="md:col-span-9 space-y-4">
          ...
          <div>
            ...
            <Select value={venue} onValueChange={(v) => setVenue(v as Venue)}>
              ...
              <SelectContent className="bg-zinc-900 text-zinc-100 border-zinc-800">
                {VENUES.map(v => <SelectItem key={v} value={v} onSelect={() => setVenue(v as Venue)}>{v}</SelectItem>)}
              </SelectContent>
            </Select>
          </div>
          <div>
        ...
        <div className="space-y-3">
          {Array.from(grouped).map(([cat, items]) => (
            <div key={cat} className="bg-zinc-900 border border-zinc-800 rounded-2xl p-0">
                      ...
                      <div key={row.checkpoint} className="grid md:grid-cols-5 gap-3 bg-zinc-900">
                        ...
                        <div>
                          ...
                          <Select value={rows[index].rating} onValueChange={(v) => setRow(index, { rating: v as Rating })}>
                            ...
                            <SelectContent className="bg-zinc-900 text-zinc-100 border-zinc-800">
                              {["Pass","Minor","Major","Critical","N/A"].map(r => <SelectItem key={r} value={r} onSelect={() => setRow(index, { rating: r as Rating })}>{r}</SelectItem>)}
                            </SelectContent>
                          </Select>
                        </div>
                        <div>
                        ...
                        {(() => {
                          ...
                          return needs ? (
                            <div>
                              ...
                              <Input type="file" accept="image/*" onChange={(e) => e.target.files?.[0] && handleFile(e.target.files[0], index)} className={`rounded-xl ${classField()}`} />
                            </div>
                          ) : null;
                        })()}
                      </div>
...
function DeepCleanSchedule() {
  ...
  const add = () => { if (!title || !due) return;
    setItems(prev => [...prev, { id: Math.random().toString(36).slice(0,9), title, due, done: false }]);
    setTitle("");
  };
...
export default function Page() {
  ...
  const onFinalise = (newActions: any[], audit: any, pdfUrlFromAudit: string) => {
        ...
        {route !== "splash" && (
          <>
            ...
            <Select value={venue} onValueChange={(v) => setVenue(v as any)}>
              ...
              <SelectContent className="bg-zinc-900 text-zinc-100 border-zinc-800">
                <SelectItem value="All" onSelect={() => setVenue("All")}>All Venues</SelectItem>
                {VENUES.map(v => <SelectItem key={v} value={v} onSelect={() => setVenue(v)}>{v}</SelectItem>)}
              </SelectContent>
            </Select>
          </>
        )}
